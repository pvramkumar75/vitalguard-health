        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between border-t border-slate-100 pt-1 px-1">
            <div className="text-xs font-semibold text-slate-400 flex items-center gap-3">
              ...
              {!isUploading && !micError && <span className="flex items-center gap-1.5 hover:text-blue-500 transition-colors cursor-help"><div className="w-1.5 h-1.5 rounded-full bg-emerald-400"></div> AI Diagnostic System Active</span>}
            </div>
            
            <button
              onClick={onFinish}
              disabled={messages.length < 2 || isProcessing}
              className="group flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-bold uppercase tracking-wider text-slate-600 hover:text-red-600 hover:bg-red-50 transition-all duration-300 disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:text-slate-600"
            >
